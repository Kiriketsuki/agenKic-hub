...
- No argument: verify the most recently modified `.html` file -- check `./docs/html/` first, then fall back to `/tmp/visual-explainer/` (`ls -t ./docs/html/**/*.html /tmp/visual-explainer/*.html 2>/dev/null | head -1`)
