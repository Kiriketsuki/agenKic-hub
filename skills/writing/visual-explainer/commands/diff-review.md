...
9. **Decision log** -- for each significant design choice in the diff, a styled card with:
   ...
   - Visual treatment by confidence level: **High** (sourced from docs): green left border. **Medium** (inferred from code): blue left border, labeled "inferred." **Low** (not recoverable): amber left border, "rationale not recoverable -- document before committing" warning.
10. **Re-entry context** -- a concise "note from present-you to future-you". *Visual treatment: compact -- consider `<details>` collapsed by default for pages with many sections.*
...
Include responsive section navigation. Use diff-style visual language: red for removed/before, green for added/after, yellow for modified, blue for neutral context. If `--publish` is specified, write to `./docs/html/reviews/`. Otherwise write to `/tmp/visual-explainer/`. Open the result in the browser and tell the user the file path.
